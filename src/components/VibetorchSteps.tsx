...
const VibetorchSteps: React.FC = () => {
  ...
  const loadRepositories = async () => {
    ...
    try {
      ...
      setRepositories(repos.slice(0, 10)); // Limit to first 10 repos
    } catch (error) {
              ...
              <>
                ...
                <div className="space-y-3">
                  <Button 
                    ...
                    className="w-full py-4 px-6 bg-cta-500 hover:bg-cta-600 text-white font-medium transition-all duration-200 hover:transform hover:-translate-y-1 hover:shadow-lg"
                  >
                    Continue to Setup →
                  </Button>
                  <Button 
        ...
        <div className="w-full">
          <div className="border-t border-b border-border p-10 bg-background">
            ...
            <div className="bg-muted rounded-lg p-4 mb-6">
              <div className="text-sm font-medium mb-3 text-foreground flex items-center justify-between">
                <span>Repository Status ({repositories.length} connected)</span>
                {loadingRepos && (
                  <div className="w-4 h-4 border-2 border-cta-500 border-t-transparent rounded-full animate-spin" />
                )}
              </div>
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {repositories.length > 0 ? (
                  repositories.slice(0, 5).map((repo) => (
                    <div key={repo.id} className="flex items-center justify-between text-sm text-muted-foreground">
                      <div className="flex items-center min-w-0 flex-1">
                        <div 
                          className={`w-2 h-2 rounded-full mr-3 flex-shrink-0 ${
                            sliderMode === 0 ? 'bg-muted-foreground' : 
                            sliderMode === 1 ? 'bg-amber-500' : 
                            sliderMode === 2 ? 'bg-emerald-500' :
                            'bg-emerald-500'
                          }`}
                        />
